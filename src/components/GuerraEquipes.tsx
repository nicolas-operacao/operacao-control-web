...
export function GuerraEquipes({ refreshTrigger, isAdmin = false }: GuerraEquipesProps) {
  ...
  const fetchRankingEDesafio = useCallback(async () => {
    ...
    try {
      ...
      // Backend retorna { data: [...], total_geral_mes } desde a última atualização
      const allUsers: VendedorRank[] = Array.isArray(rankingRes.data) ? rankingRes.data : (rankingRes.data.data || []);
      const totalGeralMes: number = rankingRes.data?.total_geral_mes ?? 0;

      setTotalTodasVendas(totalGeralMes);

      const norm = (eq: string) => String(eq || '').trim().toUpperCase();
      const a = allUsers.filter(v => ['A','EQUIPE A','EQUIPA A'].includes(norm(v.equipe))).sort((x, y) => y.total_vendido - x.total_vendido);
      const b = allUsers.filter(v => ['B','EQUIPE B','EQUIPA B'].includes(norm(v.equipe))).sort((x, y) => y.total_vendido - x.total_vendido);
