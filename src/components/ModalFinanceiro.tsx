...
export function ModalFinanceiro({ isOpen, onClose, vendas }: Props) {
  // Pega o mês atual como padrão (Ex: 2026-04)
  const dataAtual = new Date();
  // Se estivermos nos primeiros 5 dias do mês, ele puxa o mês anterior automaticamente
  ...
  const [comissaoPercent, setComissaoPercent] = useState<number>(30); // Comissão padrão (ex: 30%)

  const formataBRL = (valor: number) => valor.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });

  // Motor de Fechamento Financeiro
  const stats = useMemo(() => {
    ...
    // 2. Converte para lista e calcula comissões
    const listaSellers = Object.entries(ranking)
      .map(([nome, dados]) => {
        // Se for "Checkout Automático", a comissão é 0 (vai pra empresa)
        const isCheckout = nome === 'Checkout Automático';
        const comissao = isCheckout ? 0 : dados.total * (comissaoPercent / 100);
  ...
  }, [vendas, mesSelecionado, comissaoPercent]);
  ...
  return (
    <div className="fixed inset-0 bg-black/95 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-zinc-900 border border-green-500/30 rounded-2xl w-full max-w-5xl shadow-2xl animate-in zoom-in duration-150 flex flex-col max-h-[90vh]">
        ...
        {/* PAINEL DE CONTROLE */}
        <div className="p-6 bg-zinc-950 border-b border-zinc-800 flex flex-col md:flex-row gap-6 items-end">
          <div className="w-full md:w-auto">
            <label className="block text-zinc-400 text-[10px] font-black uppercase tracking-widest mb-2">Selecione o Mês</label>
            <input 
              type="month" 
              value={mesSelecionado} 
              onChange={(e) => setMesSelecionado(e.target.value)} 
              className="w-full bg-zinc-900 border border-zinc-700 text-white rounded-lg p-3 focus:outline-none focus:border-green-500 [color-scheme:dark] font-bold tracking-widest"
            />
          </div>
          <div className="w-full md:w-auto">
            <label className="block text-zinc-400 text-[10px] font-black uppercase tracking-widest mb-2">Taxa de Comissão (%)</label>
            <div className="flex items-center">
              <input 
                type="number" 
                value={comissaoPercent} 
                onChange={(e) => setComissaoPercent(Number(e.target.value))} 
                className="w-24 bg-zinc-900 border border-zinc-700 text-white rounded-l-lg p-3 focus:outline-none focus:border-green-500 font-black text-center"
              />
              <span className="bg-zinc-800 border-y border-r border-zinc-700 text-zinc-400 p-3 rounded-r-lg font-bold">%</span>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-green-950/20 border border-green-500/20 p-6 rounded-xl flex items-center justify-between">
            <div>
            ...
            <table className="w-full text-left">
              <thead className="sticky top-0 bg-zinc-900 backdrop-blur">
                <tr className="text-zinc-500 text-[10px] uppercase tracking-widest border-b border-zinc-800">
                  ...
                  <th className="p-4 font-black text-right">Valor Faturado</th>
              ...
              <tbody className="text-sm bg-zinc-950">
                {stats.listaSellers.length === 0 ? (
                  <tr><td colSpan={4} className="p-8 text-center text-zinc-600 font-bold uppercase tracking-widest">Nenhuma venda aprovada neste mês.</td></tr>
                ) : (
                  stats.listaSellers.map((seller, idx) => (
                    <tr key={idx} className={`border-b border-zinc-800/50 hover:bg-zinc-900/50 transition-colors ${seller.isCheckout ? 'opacity-70' : ''}`}>
                      <td className="p-4 flex items-center gap-2">
                        {seller.isCheckout ? '🤖' : '🪖'} 
                        <span className={`font-black uppercase text-xs ${seller.isCheckout ? 'text-zinc-400' : 'text-white'}`}>
                          {seller.nome}
                        </span>
                        {seller.isCheckout && <span className="bg-zinc-800 text-zinc-500 text-[8px] px-2 py-0.5 rounded uppercase tracking-widest ml-2">Empresa</span>}
                      </td>
                      <td className="p-4 text-center font-bold text-zinc-400">{seller.qtd}</td>
                      ...
                      <td className="p-4 text-right font-black text-green-400 bg-green-950/10">
                        {seller.isCheckout ? (
                          <span className="text-zinc-600 text-xs tracking-widest">-- CAIXA --</span>
                        ) : (
