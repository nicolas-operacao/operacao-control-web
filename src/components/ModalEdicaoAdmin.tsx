...
export function ModalEdicaoAdmin({ isOpen, onClose, venda, produtos, onSuccess }: Props) {
  ...
  const isCheckout = !venda.seller_id || String(venda.seller_id) === '' || String(venda.seller_id) === 'null';
  ...
  return (
    <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
      <div className="bg-zinc-900 border border-blue-500/30 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-xl shadow-2xl animate-in slide-in-from-bottom-4 sm:zoom-in duration-200 max-h-[95dvh] overflow-y-auto overscroll-contain">
        ...
        <div className="p-5 border-b border-zinc-800 flex justify-between items-center bg-blue-900/10">
          <div>
            <h2 className="text-xl font-black text-blue-400 uppercase flex items-center gap-2">✏️ Editar Venda Global</h2>
            {isCheckout && <p className="text-yellow-400 text-[10px] font-black uppercase tracking-widest mt-0.5 animate-pulse">⚠️ Venda sem vendedor — atribua abaixo</p>}
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          {/* Vendedor */}
          <div className={`rounded-xl p-3 border ${isCheckout ? 'border-yellow-500/40 bg-yellow-950/20' : 'border-zinc-800 bg-zinc-950/40'}`}>
            <label className="block text-[10px] font-black uppercase tracking-widest mb-1.5 text-yellow-400">👤 Vendedor</label>
            <select
              value={formData.seller_id}
              onChange={e => setFormData({ ...formData, seller_id: e.target.value })}
              className="w-full bg-zinc-950 border border-zinc-700 rounded-lg p-2.5 text-sm text-white cursor-pointer focus:border-yellow-400 outline-none"
            >
              <option value="">— Sem vendedor (Checkout) —</option>
              {vendedores.map(v => (
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={formData.product_name} onChange={e => setFormData({ ...formData, product_name: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg p-2.5 text-sm text-white cursor-pointer">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="date" value={formData.sale_date} onChange={e => setFormData({ ...formData, sale_date: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg p-2.5 text-sm text-white [color-scheme:dark]" />
            </div>
            <div>
              <label className="block text-zinc-500 text-[10px] font-black uppercase mb-1">Pagamento</label>
              <select value={formData.payment_method} onChange={e => setFormData({ ...formData, payment_method: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg p-2.5 text-sm text-white">
          ...
          <div>
            ...
            <input type="text" required value={formData.customer_name} onChange={e => setFormData({ ...formData, customer_name: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg p-2.5 text-sm text-white" />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-zinc-500 text-[10px] font-black uppercase mb-1">WhatsApp / Telefone</label>
              <input type="text" value={formData.customer_phone} onChange={e => setFormData({ ...formData, customer_phone: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg p-2.5 text-sm text-white" />
            </div>
            <div>
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button onMouseEnter={somHover} onClick={somClick} disabled={isLoading} className="w-2/3 bg-blue-600 hover:bg-blue-500 text-white font-black py-4 rounded-xl uppercase tracking-widest shadow-[0_0_15px_rgba(37,99,235,0.3)] transition-all text-xs">
