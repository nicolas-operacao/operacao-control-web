    ...
    <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
      <div className="bg-zinc-900 border border-blue-500/30 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-xl shadow-2xl animate-in slide-in-from-bottom-4 sm:zoom-in duration-200 max-h-[95dvh] overflow-y-auto overscroll-contain">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div className="flex gap-3 pt-4"><button type="button" onMouseEnter={somHover} onClick={() => { somClick(); onClose(); }} className="w-1/3 border border-zinc-700 hover:bg-zinc-800 text-white font-bold py-4 rounded-xl uppercase tracking-widest transition-colors text-xs">Cancelar</button><button onMouseEnter={somHover} onClick={somClick} disabled={isLoading} className="w-2/3 bg-blue-600 hover:bg-blue-500 text-white font-black py-4 rounded-xl uppercase tracking-widest shadow-[0_0_15px_rgba(37,99,235,0.3)] transition-all text-xs">{isLoading ? 'SALVANDO...' : 'SALVAR ALTERAÇÕES'}</button></div>
        </form>
      </div>
    </div>
  );
